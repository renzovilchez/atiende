...
async function getLayout(req, res, next) {
  try {
    const layout = await tenantService.getLayout(req.params.id);
...
async function getMyLayout(req, res, next) {
  try {
    const layout = await tenantService.getLayout(req.tenantId);
    res.json({ success: true, data: layout });
  } catch (err) {
    next(err);
  }
