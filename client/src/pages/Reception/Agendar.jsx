...
export default function RecepcionAgendar() {
    ...
    const { data: patients = [] } = useQuery({
        ...
        queryFn: () => api.get(`/patients?search=${patientSearch}`).then(r => r.data.data),
    ...
    const { data: doctors = [] } = useQuery({
        ...
        queryFn: () => api.get(`/specialties/${form.specialty_id}/doctors`).then(r => r.data.data),
    ...
    const { data: availability } = useQuery({
        ...
        queryFn: () => api.get(`/doctors/${form.doctor_id}/availability?date=${form.date}`).then(r => r.data.data),
        ...
        return (
            <div className="max-w-lg mx-auto text-center py-16">
                ...
                <div className="flex gap-3 justify-center">
                    <button
                        onClick={() => { setSuccess(null); setForm({ patient_id: '', specialty_id: '', doctor_id: '', date: addDays(today(), 1), notes: '' }); setSelectedPatient(null); setPatientSearch('') }}
    ...
    return (
        <div className="max-w-2xl mx-auto">
            ...
            <div className="mb-8">
                ...
                <p className="text-gray-500 mt-1">Registra una nueva cita para un paciente</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-200 p-8 space-y-6">
                ...
                <div className="relative">
                    ...
                    <input
                        type="text"
                        value={patientSearch}
                        onChange={e => { setPatientSearch(e.target.value); setShowPatients(true); if (!e.target.value) { setSelectedPatient(null); set('patient_id', '') } }}
                        ...
                        <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
                            {patients.map(p => (
                                ...
                                >
                                    ...
                                    <span className="text-sm text-gray-400">{p.dni || p.email}</span>
                                </button>
                            ))}
                        </div>
                    )}
                    {selectedPatient && (
                        <div className="mt-2 flex items-center gap-2 px-3 py-2 bg-blue-50 rounded-lg">
                            ...
                            {selectedPatient.dni && <span className="text-xs text-blue-500">DNI: {selectedPatient.dni}</span>}
                        </div>
                    )}
                </div>
                ...
                {/* Disponibilidad */}
                {availability && form.doctor_id && (
                    <div className={`p-4 rounded-xl border ${availability.available ? 'bg-green-50 border-green-200' : 'bg-orange-50 border-orange-200'}`}>
                ...
                <button
                    type="submit"
                    disabled={!form.patient_id || !form.doctor_id || !form.date || bookMutation.isPending}
