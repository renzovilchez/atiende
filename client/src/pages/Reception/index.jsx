        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
            <div style={{ background: '#fff', borderRadius: '12px', padding: '28px', width: '420px', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
                ...
                <p style={{ margin: '0 0 16px', fontSize: '13px', color: '#64748b' }}>
                    {appointment.patient_first_name} {appointment.patient_last_name} — N° {appointment.queue_position}
                </p>
                <textarea
                    value={reason}
...
function AppointmentRow({ appt, onConfirm, onCancel, onHistory }) {
    ...
    return (
        <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
            <td style={{ padding: '14px 16px', fontSize: '15px', fontWeight: 700, color: '#1a3a5c', width: '48px' }}>
                #{appt.queue_position}
            </td>
            <td style={{ padding: '14px 16px' }}>
                ...
                {appt.notes && <div style={{ fontSize: '12px', color: '#94a3b8', marginTop: '2px' }}>{appt.notes}</div>}
            </td>
            <td style={{ padding: '14px 16px' }}>
                <div style={{ fontSize: '13px', color: '#475569' }}>
                    Dr. {appt.doctor_last_name}
                </div>
                {appt.specialty_name && <div style={{ fontSize: '12px', color: '#94a3b8' }}>{appt.specialty_name}</div>}
            </td>
            <td style={{ padding: '14px 16px' }}>
                {appt.room_name && <div style={{ fontSize: '13px', color: '#475569' }}>{appt.room_name}</div>}
            </td>
            <td style={{ padding: '14px 16px' }}>
            ...
            <td style={{ padding: '14px 16px' }}>
                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                    ...
                    {canCancel && (
                        <button onClick={() => onCancel(appt)} style={btnStyle('#dc2626', true)}>
                            Cancelar
                        </button>
                    )}
                    <button onClick={() => onHistory(appt)} style={btnStyle('#64748b', true)}>
                        Historial
                    </button>
                </div>
            </td>
                            ...
                            <div key={event.id} style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                                ...
                                <div>
                                    ...
                                    <div style={{ fontSize: '11px', color: '#94a3b8', marginTop: '2px' }}>
                                        {event.first_name ? `${event.first_name} ${event.last_name} · ` : ''}
...
export default function Recepcion() {
    ...
    const { data, isLoading, error } = useQuery({
        ...
        queryFn: () => api.get(`/appointments?date=${date}`).then(r => r.data.data),
    })
    ...
    return (
        <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
                <div>
                    <h1 style={{ margin: 0, fontSize: '22px', fontWeight: 700, color: '#0f172a' }}>Citas del día</h1>
                    <p style={{ margin: '4px 0 0', fontSize: '14px', color: '#64748b' }}>Panel de recepción</p>
                </div>
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                    <button
                        onClick={() => navigate('/recepcion/pacientes')}
                        style={{ padding: '8px 14px', border: '1px solid #e2e8f0', borderRadius: '8px', background: '#fff', fontSize: '13px', cursor: 'pointer', color: '#475569', fontWeight: 500 }}
                    >
                        👤 Pacientes
                    </button>
                    <button
                        onClick={() => navigate('/recepcion/agendar')}
                        style={{ padding: '8px 14px', border: 'none', borderRadius: '8px', background: '#2563eb', fontSize: '13px', cursor: 'pointer', color: '#fff', fontWeight: 500 }}
                    >
                        + Agendar
                    </button>
                    <input
                        ...
                        style={{ padding: '8px 12px', border: '1px solid #e2e8f0', borderRadius: '8px', fontSize: '14px', color: '#0f172a', outline: 'none' }}
                    />
                </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px', marginBottom: '24px' }}>
                {[
                    ...
                    <div key={s.label} style={{ background: '#fff', border: '1px solid #e8edf2', borderRadius: '10px', padding: '16px', textAlign: 'center' }}>
            ...
            <div style={{ background: '#fff', border: '1px solid #e8edf2', borderRadius: '12px', overflow: 'hidden' }}>
                {isLoading ? (
                    <div style={{ padding: '48px', textAlign: 'center', color: '#94a3b8' }}>Cargando citas...</div>
                ) : error ? (
                    <div style={{ padding: '48px', textAlign: 'center', color: '#dc2626' }}>Error al cargar citas</div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <thead>
                            <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e8edf2' }}>
                                {['N°', 'Paciente', 'Doctor', 'Consultorio', 'Estado', 'Acciones'].map(h => (
