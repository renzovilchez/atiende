...
function StepDoctorFecha({ specialty, onSelect, onBack, isPatient }) {
  ...
  const { data: doctors = [], isLoading: loadingDoctors } = useQuery({
    ...
    queryFn: () =>
      api.get(`/specialties/${specialty.id}/doctors`).then((r) => r.data.data),
  });
  ...
  const { data: availability, isLoading: loadingAvail } = useQuery({
    ...
    queryFn: () =>
      api
        .get(`/doctors/${selectedDoctor.id}/availability?date=${date}`)
        .then((r) => r.data.data),
  ...
  return (
    <div className={`animate-fadeIn ${animate ? "opacity-100" : "opacity-0"}`}>
      <div className="text-center mb-8">
        ...
        <p className="text-gray-600">
          Especialidad:{" "}
          <span className="font-semibold text-blue-600">{specialty.name}</span>
        </p>
      </div>
                  ...
                  <div className="flex-1 text-left">
                    ...
                    {d.license_number && (
                      <p className="text-sm text-gray-500">
                        CMP {d.license_number}
                      </p>
                    )}
                  </div>
                  {selectedDoctor?.id === d.id && <CheckIcon />}
                </div>
              </button>
            ))}
          </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-orange-600">
                            Se agendará como paciente adicional
                          </p>
                        </div>
                      </div>
                    )}
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          className={`
                        ...
                        ${
                          selectedDoctor &&
                          (!isPatient || availability?.available)
    ...
    return (
      <div className="max-w-lg mx-auto text-center py-12 animate-fadeIn">
        ...
        <div className="flex gap-4 justify-center">
          ...
          <button
            onClick={() =>
              navigate(isPatient ? `/${slug}/mis-citas` : `/${slug}/recepcion`)
            }
