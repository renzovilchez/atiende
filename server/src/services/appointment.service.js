...
function validateFutureDate(date) {
  ...
  if (date < today)
    throw new AppError("No se puede agendar en una fecha pasada", 400);
}
...
async function book(tenantId, data, createdBy, createdByRole) {
  ...
  const doctor = await repo.findDoctor(data.doctor_id);
  if (!doctor)
    throw new AppError(
      "Doctor no encontrado o no pertenece a esta clínica",
      404,
    );

  const schedule = await repo.getScheduleForDate(data.doctor_id, data.date);
  if (!schedule)
    throw new AppError("El doctor no tiene turno en esa fecha", 400);
  ...
  if (isFull && createdByRole === "patient") {
    throw new AppError(
      `No hay cupos disponibles. Máximo: ${schedule.max_patients}`,
      409,
    );
  }
  ...
  if (existing)
    throw new AppError(
      "El paciente ya tiene una cita con este doctor en esa fecha",
      409,
    );

  const queuePosition = await repo.nextQueuePosition(data.doctor_id, data.date);

  return repo.bookTransaction(tenantId, {
    ...data,
...
async function reschedule(tenantId, appointmentId, data, changedBy) {
  ...
  if (["completed", "cancelled", "in_progress"].includes(appointment.status)) {
    throw new AppError(
      `No se puede reagendar una cita en estado: ${appointment.status}`,
      400,
    );
  }
  ...
  return repo.rescheduleTransaction(tenantId, appointmentId, {
    doctor_id: doctorId,
    date: data.date,
    queue_position: queuePosition,
    schedule_id: schedule.id,
...
async function getHistory(tenantId, appointmentId) {
  ...
  const appointment = await repo.findById(appointmentId);
  if (!appointment) throw new AppError("Cita no encontrada", 404);
  return repo.findHistory(appointmentId);
}
